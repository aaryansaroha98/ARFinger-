...
class ARFingerApp {
    ...
    processResults(results) {
        // Clear previous frame drawings on overlay canvas (not the persistent drawing canvas)
        this.overlayCtx.clearRect(0, 0, this.width, this.height);
        ...
        if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            // Draw or erase on drawing canvas (persistent)
            if (currentPoint) {
    ...
    draw(point, gesture) {
        if (gesture === 'draw' && this.prevPoint) {
            // Draw line with custom color on drawing canvas
